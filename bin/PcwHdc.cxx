 ...
 * DRM=0x3FF (1023)
 * */
static const zbyte mzdpb[] = { 0x80, 0x00, 0x05, 0x1F, 0x01, 0xFF, 0x07, 0xFF,
			      0x03, 0xFF, 0x00, 0x00, 0x80, 0x00, 0x00, 0x02,
			      0x03 };
// CPC system format 
static const zbyte sysdpb[]= { 0x24, 0x00, 0x03, 0x07, 0x00, 0xAA, 0x00, 0x3F, 
	                      0x00, 0xC0, 0x00, 0x10, 0x00, 0x02, 0x00, 0x02,
			      0x03 };
// CPC data format
static const zbyte datadpb[]={ 0x24, 0x00, 0x03, 0x07, 0x00, 0xB3, 0x00, 0x3F, 
...
UiEvent PcwHDC::settings(UiDrawer *d)
{
	...
	while(1)
	{
		...
		uim.add(new UiSeparator(d));
		uim.add(new UiSetting(SDLK_8, 
			  m_all8mb, "  All drives use fixed 8Mb format  ", d));
		uim.add(new UiLabel("  (this allows more than 6 drives  ", d));
		uim.add(new UiLabel("   to be mapped under CP/M)", d));
		uim.add(new UiSeparator(d));
		for (n = 2; n < 16; n++)
		{
...
{
	char *s = (char *)xmlGetProp(cur, (xmlChar *)"all8mb");
	if (s) m_all8mb = atoi(s);
...
bool PcwHDC::storeNode(xmlDocPtr doc, xmlNsPtr ns, xmlNodePtr cur)
{
	...
	sprintf(buf, "%d", m_all8mb);
	xmlSetProp(cur, (xmlChar *)"all8mb", (xmlChar *)buf);
        for (int n = 0; n < 16; n++)
        {
                some = false;
                sprintf(buf, "%c", n + 'a');
...
void PcwHDC::fidPuts(zword addr, const string s)
{
...
{	// or hard, and set HL IX IY accordingly 
	...
	if (m_all8mb || isHard(m_drvName[img], m_drvType[img]))	// Fixed
	{
		R->setHL(513);
		R->ix = 0;
		R->iy = 0;
...
*/
	...
	switch (R->d)
	{
		...
		R->b = cdrv;
		setupPars(R);
/* The all-8mb hack: If we know all drives allocated by JOYCE will have 
 * exactly the same geometry, we don't need a DPB for each one. PCW CP/M 
 * allows up to 6 drives to be added, but the constraint is allocating the
 * DPBs in common memory. Unfortunately there's no way to tell SVC_D_HOOK
 * that it doesn't have to allocate the DPB; so let's do it by hand. */
		if (m_all8mb)
		{
			all8mhack(R);
...
void PcwHDC::all8mhack(Z80 *R)
{
	zword drvtbl, dph, dpb;
	
/* 1. See if this is CP/M, by checking for the BIOS jumpblock */
	for (int n = 0xFC00; n <= 0xFC60; n += 3)
	{
		if (m_sys->m_cpu->fetchB(n) != 0xC3) 
		{
			return;
		}
	}
/* 2. Locate the DPH of the drive that was just added (E=letter) */
	drvtbl = m_sys->m_cpu->fetchW(1 + m_sys->m_cpu->fetchW(0xFC43));
	dph    = m_sys->m_cpu->fetchW(drvtbl+2*R->e);
	dpb    = m_sys->m_cpu->fetchW(dph+12);

/*	fprintf(stderr, "drvtbl=%04x dph=%04x dpb=%04x\n", drvtbl, dph, dpb);
 *
 * 3. Now locate the piece of code in the FID loader that allocates a new 
 *   DPB... 
 *    11 11 00    ld      de, 11h
 *    cd 76 f2    call    svc_alloc_tpa
 *    d2 37 f1    jp      nc, svc_d_hook_6
 *    fd 75 10    ld      (iy+10h), l
 *    fd 74 11    ld      (iy+11h), h
 */
	for (int n = 0xFFF0; n >= 0; n--)
	{
		if (m_sys->m_cpu->fetchB(n)    == 0x11 && 
		    m_sys->m_cpu->fetchB(n+ 1) == 0x11 && 
		    m_sys->m_cpu->fetchB(n+ 2) == 0x00 && 
		    m_sys->m_cpu->fetchB(n+ 9) == 0xFD && 
		    m_sys->m_cpu->fetchB(n+10) == 0x75 && 
		    m_sys->m_cpu->fetchB(n+11) == 0x10 && 
		    m_sys->m_cpu->fetchB(n+12) == 0xFD && 
		    m_sys->m_cpu->fetchB(n+13) == 0x74 && 
		    m_sys->m_cpu->fetchB(n+14) == 0x11)
		{
/*			fprintf(stderr, "Drive hook code at %04x\n", n); */

			m_sys->m_cpu->storeB(n+3, 0x21); /* LD HL, DPB */
			m_sys->m_cpu->storeW(n+4, dpb);
			m_sys->m_cpu->storeB(n+6, 0x37); /* SCF */
			m_sys->m_cpu->storeB(n+7, 0x00); /* NOP */
			m_sys->m_cpu->storeB(n+8, 0x00); /* NOP */
		}
	}
...
static zword to_al0(zword blocks)
{
	zword al0 = 0;
...
void PcwHDC::makeDpb(zword dpb, zbyte *format, int is_xdpb)
{
	zword BLOCK_SIZE, OFF, NUM_TRACKS, NUM_SECTORS, DIR_BLOCKS,
	     SEC_SIZE;

	zword SPT, BSH, BLM, DSM, DRM, AL0, AL1, EXM, CKS, PSH, PHM;
