...
void txt2fcb(Z80 *R, zword addr, char *s)	/* Parse ASCII filename to FCB */
{
	char curch;
	int fcnt = 0;
...
zword jdos_creat(Z80 *R, JDOSMode mode)	/* Create a file, DE->name */
{
	int n, m;
	char c;
...
zword jdos_open(Z80 *R, JDOSMode mode)	/* Open a file, DE->name */
{
	int n, m;
	char c;
...
zword jdos_putc(Z80 *R, JDOSMode mode)	/* Write byte to file */
{
	int n = R->getDE();
...
zword jdos_getc(Z80 *R, JDOSMode mode)	/* Read a byte from a file into B */
{
	int n = R->getDE();
	int m;
...
zword jdos_close(Z80 *R, JDOSMode mode)	/* Close file */
{
	int n = R->getDE();
	int m;
