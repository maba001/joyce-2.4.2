...
static void write_pcwlink(Z80 *r, zbyte v)
{
	FILE *fp;
...
static zbyte read_pcwlink(Z80 *r)
{
	zbyte v;
	FILE *fp;
...
static int id = 0;
/*
static void daisyIn(zbyte hi, zbyte lo, zword pc, zbyte a, zbyte l, zword sp)
{
	int n;
...
static void daisyOut(zbyte hi, zbyte lo, zbyte v, zword pc)
{
...
/*
 ...
 //
	{
		static zword w = 0;
		zword f2 = PCWRAM[0x1024] + 256 * PCWRAM[0x1025];
...
int JoyceZ80::interrupt(void)
{
	...
	{
		m_cenCount = 0;
		m_sys->m_cen.libLinkTick();
		m_sys->m_cps.libLinkTick();
		m_sys->m_locoLink.libLinkTick();
	}
...
zbyte JoyceZ80::fetchB(zword addr) { return fetch(addr); }
zword JoyceZ80::fetchW(zword addr) { return fetch2(addr); }
void JoyceZ80::storeB(zword addr, zbyte v) { store(addr, v); }
void JoyceZ80::storeW(zword addr, zword v) { store2(addr, v); }
