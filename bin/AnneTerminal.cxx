...
static zbyte        gl_screenShadow[80][480];
static int          gl_lineLen;
static zbyte        gl_shadowInvalid;
...
void AnneTerminal::out(zword port, zbyte value)
{
	/* Palette bits */
	port &= 0xFF;
...
void AnneTerminal::border(zbyte forced)
{
        int count, n;
        SDL_Rect rects[4];
        Uint32 pixel;
	...
	else
	{
        ...
        if (!m_visible || !(m_scrEnabled & 1)) return;
...
void AnneTerminal::setForeground(zbyte r, zbyte g, zbyte b) 
{
        m_inks[1].r = r;
	m_inks[1].g = g;
	m_inks[1].b = b;
...
void AnneTerminal::setBackground(zbyte r, zbyte g, zbyte b)
{
        m_inks[0].r = r;
	m_inks[0].g = g;
	m_inks[0].b = b; 
                               ...
                               unsigned xto, unsigned yto)
{
	...
	int step;
	zbyte *dataFrom, *dataTo;
	zword rrvalue;
	zbyte maskFrom, maskTo;
	zbyte buf[80];
...
// Greyscale patterns
static zbyte g0[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };	// 00
static zbyte g1[] = { 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x44, 0x00 };	// 01
static zbyte g2[] = { 0x44, 0x00, 0x44, 0x00, 0x44, 0x00, 0x44, 0x00 };	// 02
static zbyte g3[] = { 0x44, 0x11, 0x44, 0x11, 0x44, 0x11, 0x44, 0x11 };	// 03
static zbyte g4[] = { 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55 };	// 04
static zbyte g5[] = { 0xBB, 0xEE, 0xBB, 0xEE, 0xBB, 0xEE, 0xBB, 0xEE };	// 05
static zbyte g6[] = { 0xBB, 0xFF, 0xBB, 0xFF, 0xBB, 0xFF, 0xBB, 0xFF };	// 06
static zbyte g7[] = { 0xFF, 0xFF, 0xBB, 0xFF, 0xFF, 0xFF, 0xBB, 0xFF };	// 07
static zbyte g8[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };  // 08

static zbyte *shades[9] =
{
...
bool AnneTerminal::doRect(unsigned x, unsigned y, unsigned w, unsigned h, 
	zbyte fill, zbyte rasterOp, zbyte *f)
{
	zbyte lmask = 0, rmask = 0;
	zbyte *row;
...
bool AnneTerminal::doBitmap(unsigned x, unsigned y, unsigned w, unsigned h,
                                zbyte *data, zbyte *mask, int slice)
{
	unsigned x1, y1;
