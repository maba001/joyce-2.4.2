...
{
	zbyte *chain = m_fat + ((cluster / 2) * 3);
	if (cluster & 1) 
		return (chain[2] << 4) | (chain[1] >> 4);
	else	return chain[0] | ((chain[1] & 0x0F) << 8);
}

int DosDisk::fileSize(zbyte *dirEnt)
{
...
UiEvent DosDisk::loadFile(zbyte *dirEnt, UiDrawer *d, DosFile &file)
{
	dsk_err_t err;
	int cluster = dirEnt[26] + 256 * dirEnt[27];
	int len = 0;
	...
	zbyte *data = new zbyte[len = fileSize(dirEnt)];
	zbyte *ptr = data;	
	while (cluster < 0xFF0) 
	{
