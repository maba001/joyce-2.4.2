...
int PcwLogoTerm::makerop(zbyte pen, zbyte *colour)
{
	switch(pen)
	{
		default: return 0;		/* pd */
...
void PcwLogoTerm::zstrcpy(zword dest, char *src)
{
	do
	{
...
zword PcwLogoTerm::lios_init(zword liospb, zword msgbuf)
{
	char title[90];
...
//
	...
	m_sys->m_cpu->storeW(liospb +10, (zword)m_sysVideo->colourToIndex(0xFF, 0xFF, 0xFF));
	m_sys->m_cpu->storeW(liospb +12, (zword)m_sysVideo->colourToIndex(0x00, 0x00, 0x00));
...
void PcwLogoTerm::lios3(zbyte split)
{
	m_splitSz = split;
...
void PcwLogoTerm::lios7(zword T, zword B)	/* Clear text lines T to B */
{
	m_term->clearLineRange(T-1, B-1);
}

void PcwLogoTerm::lios8(zword X, zword Y)  /* Move text cursor to X,Y */
{
	m_term->moveCursor(X - 1, Y - 1);
}


void PcwLogoTerm::lios9(zword T, zword B) /* Scroll text window */
{
	m_term->scrollLineRangeUp(T - 1, B - 1);
	m_term->clearCurLine();
}

zword PcwLogoTerm::lios10(zword X, zword Y, zbyte colour, zbyte pen)
{
...
zword PcwLogoTerm::lios11(zword X1, zword Y1, zword X2, zword Y2, zbyte colour, zbyte pen)
{
	int rop = makerop((pen == 2) ? 3 : 1, &colour);	

        GrLineRop(X1, Y1, X2, Y2, colour, rop);

	if (X1 > X2) { zword t = X2; X2 = X1; X1 = t; }
	if (Y1 > Y2) { zword t = Y2; Y2 = Y1; Y1 = t; }
...
zword PcwLogoTerm::lios13(zword colour, zword pixels)
{
...
zword PcwLogoTerm::lios16(zword X, zword Y)
{
	return (zword)(GrPixel(X, Y));
}


zword PcwLogoTerm::lios24(zword buf, zword recno)
{
	return get_bmp_record(buf, recno);
}

zword PcwLogoTerm::lios25(zword buf, zword recno)
{
...
zword PcwLogoTerm::get_bmp_record(zword Z80addr, zword recno)
{
	unsigned int n;
...
zword PcwLogoTerm::put_bmp_record(zword Z80addr, zword recno)
{
	int bmpptr, z80ptr = 0;
	int where = 128 * recno;
