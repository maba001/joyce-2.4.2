...
// Redefine a group of keys
	void setRange(int *R, zbyte a, zbyte b, zbyte c, zbyte d);

// Apply latest settings
	void keySet(void);
// Keyboard returns a code
	virtual void pushKey(zbyte k);
	void onCommand(void);

// Swap ALT / DELETE settings
	bool	m_keyPress;
	std::list<zbyte> m_queue;	// Outgoing keys
	zbyte	m_command;		// Incoming command
	zbyte	m_cmdActive;		// Active command
	zbyte	m_leds;			// LED status
	zbyte	m_typematic;		// Typematic delay/option byte
	zbyte	m_scanSet;		// What scancode set to use
