...
public:
	...
	void setMotor(zbyte b);
	void writeData(zbyte b);
	void writeDOR(int value);
	void writeDRR(zbyte value);
	zbyte readData(void);
	zbyte readControl(void);
	zbyte readDIR(void);
	void isr(FDC_PTR, int);
	bool loadBootSector(const string filename, const char *type, zbyte *sector);
...
//	FDRV_PTR         m_fd[4];         /* (v1.37) floppy drives A: and B: */   
	...
	zbyte m_terminalCount;             /* (v1.37) FDC's TC line */ 
	bool m_fdcInt;
