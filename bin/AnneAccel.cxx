...
// etcetera. 
//
void AnneAccel::showRosanneCall(zbyte function, zword *pbc, zword *pde, zword *phl, Z80 *R)
{
	switch(function)
	{
		static zbyte prevdev;
...
bool AnneAccel::override(zbyte function, zword *pbc, zword *pde, zword *phl, Z80 *R)
{
...
////////////////// GRAPHICS OPERATION OVERRIDES  //////////////////////////////
                ///////////////////////////////////
inline unsigned parword(zbyte **p)
{
	unsigned w = p[0][1];
	w = (w << 8) | p[0][0];
	p[0] += 2;
...
zbyte *AnneAccel::paraddr(unsigned width, unsigned height, zbyte **p)
{
	unsigned wb     = (width+7)/8;
	unsigned buflen = wb * height;
	zbyte *data      = new zbyte[buflen];
	unsigned cur 	= 0;
	unsigned addr = p[0][1];
	addr = (addr << 8) | p[0][0];
	int bank = p[0][2];
...
void AnneAccel::hook(int call, zbyte *jb)
{
	unsigned code;
	zbyte *pCode;
...
void AnneAccel::unhook(int call, zbyte *jb)
{
	unsigned code;
	zbyte *pCode;
...
void AnneAccel::bios(Z80 *R)
{
	...
	zword x, y, w, h, count, ink;
	zbyte *param, *data, *mask;
	unsigned offset;
...
bool AnneAccel::overrideGraphics(zbyte function, zword *pbc, zword *pde, zword *phl, Z80 *R)
{
	switch(function)
	{
...
bool AnneAccel::dg_blk_copy(zbyte *params, bool move)
{
...
bool AnneAccel::dg_blk_fill(zbyte *params)
{
	unsigned x, y, w, h;
	zbyte filltype = params[0];
	bool b;
	zbyte raster = 0;
...
//
bool AnneAccel::overrideFS(zbyte function, zword *pbc, zword *pde, zword *phl, Z80 *R)
{
...
void AnneAccel::os_set_device(zbyte device)
{
...
string AnneAccel::pathname(zword vhl, zword vde)
{
...
string AnneAccel::filename(zword vhl, zword vde)
{
	char *fbuf = (char *)(PCWRAM + vhl);
	char *tbuf = NULL;
	string fname;
...
void AnneAccel::os_find_first(zword vhl, zword vde, Z80 *R)
{
...
void AnneAccel::os_get_cwd(zword vhl)
{
	char *fbuf = (char *)(PCWRAM + vhl);
	int idx;
...
void AnneAccel::os_set_cwd(zword vhl, Z80 *R)
{
...
void AnneAccel::os_open_file (zword hlv, zword dev, zbyte b, Z80 *R)
{
...
bool AnneAccel::os_close_file(zbyte b, Z80 *R)
{
	FILE **ptr = (b ? &m_fss.m_fpwrite : &m_fss.m_fpread);
...
void AnneAccel::os_set_attrib(zword hlv, zword dev, zbyte b, Z80 *R)
{
	R->f |= 1;
	R->a = 5;
}
...
void AnneAccel::os_set_filedate(zword hlv, zword dev, Z80 *R)
{
	if (!m_fss.m_fpwrite)
	{
		R->f |= 1;
...
void AnneAccel::os_create_dir(zword hlv, Z80 *R)
{
...
void AnneAccel::os_remove_dir(zword hlv, Z80 *R)
{
	string filename = pathname(hlv);
...
void AnneAccel::os_delete_file(zword hlv, zword dev, Z80 *R)
{
	string filename = pathname(hlv, dev);
...
void AnneAccel::os_rename_file(zword hlv, zword dev, zword ixv, zword iyv, Z80 *R)
{
...
void AnneAccel::os_disk_space(zword *pbc, zword *phl)
{
#ifdef HAVE_WINDOWS_H
...
//
//
unsigned long AnneAccel::checksum(zbyte *rosanne, zbyte **pchksum)
{
	int off = 0x10000;
...
void AnneAccel::patch(zbyte *rosanne)
{
	int n;
	int fixsum = 0;
...
int AnneAccel::cb_get_next_mem(zbyte mem)
{
	Z80 *R = new AnneZ80(m_sys);
	// Call native os_get_mem
...
void AnneAccel::cb_typo_def(Z80 *R, zword vde, zword vhl)
{
	Z80 *R2 = new AnneZ80(m_sys);
...
zbyte AnneAccel::cb_bios_getmode()
{
	Z80 *R = new AnneZ80(m_sys);
...
zbyte AnneAccel::cb_bios_getink()
{
	Z80 *R = new AnneZ80(m_sys);
	// Call native os_get_mem
...
zbyte *AnneAccel::cb_bios_getpatt()
{
	Z80 *R = new AnneZ80(m_sys);
	// Call native os_get_mem
	z80Callback(R, 0x80 + 3 * bios_getpatt, 0);
	zbyte *rv = addr16(R->getHL());
	delete R;
	return rv;
}
...
void AnneAccel::z80Callback(Z80 *R, zword addr, zbyte a, zword vbc, zword vde, zword vhl)
{
	zbyte backup[256];
	zword oldpc, oldsp, pcode = PCODEBASE;
...
zbyte *AnneAccel::addr16(zword hlv)
{
	return anneMem[hlv >> 14] + hlv;	
}


zbyte *AnneAccel::addr24(zbyte c, zword hlv)
{
	zbyte *addr;
...
zbyte *AnneAccel::addr24(unsigned chl)
{
