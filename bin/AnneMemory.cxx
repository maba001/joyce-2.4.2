...
int AnneMemory::re_alloc(long &requested, long &actual, int &nbanks, zbyte **ram)
{
	long siz  = requested;
...
void AnneMemory::out(zbyte port, zbyte value)
{
	int bankoff;
	zbyte *mem  = (value & 0x80) ? PCWRAM   : PCWFLASH;
...
zbyte AnneMemory::in(zbyte port)
{
	return anneMemIO[port % 4];
}

zbyte AnneMemory::getPage(zbyte port)
{
	return in(port);
}
...
void flashStore(zbyte *addr, zbyte value)
{
